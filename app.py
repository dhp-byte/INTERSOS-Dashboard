    ...
    with col_c:
        st.markdown(
            ...
            <div style="display:inline-flex; align-items:center; justify-content:center;
                        ...
                        background:#FFFFFF; border:1px solid {t['border']}; padding:10px;">
                <img src="{config.LOGO_URL}" alt="INTERSOS" style="max-width:100%; max-height:100%; object-fit:contain;">
            </div>
            </div>""",
...
def render_overview(sheets: dict[str, pd.DataFrame], cover: dict[str, str]) -> None:
    ...
    st.markdown(
        hero(
            ...
            bg_image=config.HERO_BACKGROUND_IMAGE,
        ),
        unsafe_allow_html=True,
    )
    st.markdown(image_slider(config.ILLUSTRATION_IMAGES), unsafe_allow_html=True)
