"""
pages/9_Beneficiaires.py — Registre des bénéficiaires enregistrés
...
topbar.render_topbar("Bénéficiaires")
...
with col_c:
    ...
    top_prov = ben["Province"].value_counts().head(5).reset_index()
    top_prov.columns = ["Province", "Bénéficiaires"]
    st.dataframe(top_prov, use_container_width=True, hide_index=True)
    fig4 = px.bar(top_prov, x="Bénéficiaires", y="Province", orientation="h", color="Province",
                  color_discrete_sequence=["#3D6FE0", "#40C057", "#15AABF", "#F76707", "#6C5CE7"])
    fig4.update_layout(showlegend=False, height=240, yaxis={"categoryorder": "total ascending"})
    st.plotly_chart(fig4, use_container_width=True)

with col_d:
    if "Date_Registration" in ben.columns:
        ...
        fig5 = px.line(monthly_counts, x="Mois", y="Bénéficiaires", markers=True)
        fig5.update_traces(line_color="#3D6FE0")
