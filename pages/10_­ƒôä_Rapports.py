"""
pages/10_📄_Rapports.py — Génération à la demande d'un rapport de synthèse
...
with c1:
    ...
    if st.button("Générer le rapport Word", use_container_width=True, key="gen_docx"):
        with st.spinner("Génération du document Word en cours..."):
            docx_bytes = rg.build_docx_report(sheets, cover)
...
with c2:
    ...
    if st.button("Générer le rapport PDF", use_container_width=True, key="gen_pdf"):
        with st.spinner("Génération du document PDF en cours..."):
            pdf_bytes = rg.build_pdf_report(sheets, cover)
...
with st.expander("Contenu du rapport"):
    st.markdown(
        ...
        2. **Performance par secteur** — activités, cible cumulée, atteint cumulé, taux de réalisation
        3. **Suivi des indicateurs** — statut (En bonne voie / À surveiller / Retard critique) pour chacun des 24 indicateurs
