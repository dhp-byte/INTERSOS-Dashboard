...
matplotlib.use("Agg")  # backend sans interface graphique, compatible tout environnement serveur
...
BRAND_BLUE_HEX = "#1B3A6B"


def _achievement_label(pct: float, thresholds: dict) -> str:
    if pct >= thresholds["on_track_min"]:
        return "En bonne voie"
    if pct >= thresholds["watch_min"]:
        return "À surveiller"
...
def _sector_performance_chart_png(sheets: dict[str, pd.DataFrame]) -> bytes:
    """Graphique en barres du taux de réalisation par secteur, dans les
    couleurs officielles de config.SECTOR_COLORS. Retourne un PNG en mémoire."""
    rows = [r for r in _sector_summary_rows(sheets) if r["pct"] is not None]
    if not rows:
        return b""
    sectors = [r["sector"] for r in rows]
    pcts = [r["pct"] for r in rows]
    colors_list = [config.SECTOR_COLORS.get(s.replace(" ", "_"), "#4C6EF5") for s in sectors]

    fig, ax = plt.subplots(figsize=(6.4, 3.2), dpi=150)
    bars = ax.bar(sectors, pcts, color=colors_list)
    ax.axhline(100, color="#888888", linestyle="--", linewidth=0.8)
    ax.set_ylabel("Taux de réalisation (%)")
    ax.set_ylim(0, max(110, max(pcts) + 10))
...
def _indicator_status_chart_png(indicators: pd.DataFrame, thresholds: dict) -> bytes:
    """Diagramme en barres du nombre d'indicateurs par statut (bonne voie /
    à surveiller / retard critique), avec les couleurs sémantiques du thème."""
    labels_order = ["En bonne voie", "À surveiller", "Retard critique"]
    status_colors = {"En bonne voie": "#33C481", "À surveiller": "#F4A93B", "Retard critique": "#E5484D"}
    counts = indicators["Achievement_%"].apply(lambda p: _achievement_label(p, thresholds)).value_counts()
    values = [int(counts.get(lbl, 0)) for lbl in labels_order]

    fig, ax = plt.subplots(figsize=(6.4, 2.6), dpi=150)
    bars = ax.barh(labels_order, values, color=[status_colors[l] for l in labels_order])
    ax.spines[["top", "right"]].set_visible(False)
    ax.set_xlabel("Nombre d'indicateurs")
    for bar, val in zip(bars, values):
        ax.annotate(str(val), (bar.get_width(), bar.get_y() + bar.get_height() / 2),
...
# Word (.docx)
# ---------------------------------------------------------------------------
def build_docx_report(sheets: dict[str, pd.DataFrame], cover: dict[str, str], thresholds: dict | None = None) -> bytes:
    """Construit un rapport Word de synthèse exécutive et retourne ses octets."""
    thresholds = thresholds or config.ACHIEVEMENT_THRESHOLDS
    doc = Document()
    ...
    kpis = [
        ...
        (
            ...
            f"{(indicators['Achievement_%'] >= thresholds['on_track_min']).sum()}/{len(indicators)}",
        ),
    ]
    ...
    sector_chart_png = _sector_performance_chart_png(sheets)
    if sector_chart_png:
        doc.add_picture(io.BytesIO(sector_chart_png), width=Cm(15))
    ...
    for _, r in indicators.iterrows():
        ...
        row[3].text = _achievement_label(r["Achievement_%"], thresholds)

    indicator_chart_png = _indicator_status_chart_png(indicators, thresholds)
    if indicator_chart_png:
        doc.add_picture(io.BytesIO(indicator_chart_png), width=Cm(15))
...
# PDF
# ---------------------------------------------------------------------------
def build_pdf_report(sheets: dict[str, pd.DataFrame], cover: dict[str, str], thresholds: dict | None = None) -> bytes:
    ...
    sector_chart_png = _sector_performance_chart_png(sheets)
    if sector_chart_png:
        story.append(Image(io.BytesIO(sector_chart_png), width=15 * cm, height=7.5 * cm))
    story.append(Spacer(1, 14))
    ...
    for _, r in indicators.iterrows():
        ind_rows.append(
            [str(r["Indicator"]), str(r["Sector"]), f"{r['Achievement_%']:.1f}%", _achievement_label(r["Achievement_%"], thresholds)]
        )
    ...
    if indicator_chart_png:
        story.append(Spacer(1, 10))
        story.append(Image(io.BytesIO(indicator_chart_png), width=15 * cm, height=6 * cm))
